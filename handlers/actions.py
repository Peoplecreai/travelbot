...
def post_flight_buttons(datos, state, event, client, doc_ref, search_google_flights):
...
def post_hotel_buttons(datos, state, event, client, doc_ref, max_lodging, search_hotels, area=None):
    hotel_area = area or datos['destination']
    hotels = search_hotels(hotel_area, max_lodging) or []
    if not hotels:
        return None
